...
class GCN_Temp(torch.nn.Module):
    def __init__(self, initial_dim_gcn, edge_dim_feature,
        ...
        self.nn_conv_1 = ARMAConv(initial_dim_gcn, hidden_dim_nn_1,
                                num_stacks = 3, dropout=0, num_layers=7, shared_weights = False )
        self.dropout_1 = nn.Dropout(p=p1)
        
        self.nn_conv_2 = ARMAConv(hidden_dim_nn_1, hidden_dim_nn_2,
                                num_stacks = 3, dropout=0, num_layers=7, shared_weights = False)
        self.dropout_2 = nn.Dropout(p=p2)
        
        self.nn_conv_3 = ARMAConv(hidden_dim_nn_2, hidden_dim_nn_3,
                                num_stacks = 3, dropout=0, num_layers=7, shared_weights = False)
...
# num of layers should be greater than the number of stacks
    ...
    def forward(self, data):
        ...
        x = self.nn_conv_1(x, edge_index)
        x = F.relu(x)
        x = self.dropout_1(x)
        
        x = self.nn_conv_2(x, edge_index)
        x = F.relu(x)
        x = self.dropout_2(x)
        
        x = self.nn_conv_3(x, edge_index)
        x = F.relu(x)
        x = self.dropout_3(x)
